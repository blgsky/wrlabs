...
// Drops a session the caller has just found to be stale. Guarded on identity so
// that when a page's worth of requests all discover the same dead manifest at
// once, only the first one refetches it and the rest await that single result.
function invalidate(stale) {
  if (sessionPromise === stale) sessionPromise = null
}
...
async function handle(request, url) {
  ...
  // Blob ids are regenerated on every deploy, so a worker that outlives a
  // rebuild holds a manifest whose ids have all just stopped existing. That
  // reads as a 404 on the blob, not as a dead path — and nothing else here
  // would ever notice, which is how one deploy used to turn every page of a
  // long-open tab into the "not here" notice below until the worker was
  // manually unregistered. So a missing blob costs one manifest refetch and a
  // second attempt; only a path missing from the *fresh* manifest is dead.
